...
public class EidasProtocolProcessor implements ProtocolProcessorI {
    ...
    public static final int MAXIMUM_MDS_FOR_REPRESENTATION_CASE = 2;
     ...
     */
    @Override
    public boolean checkRepresentationResponse(@Nonnull ImmutableAttributeMap immutableAttributeMap) {
        Set<PersonType> dataSets = immutableAttributeMap.getDefinitions().stream()
                .filter((attr) -> EidasSpec.REGISTRY.getAttributes().contains(attr))
                .distinct()
                .map(AttributeDefinition::getPersonType)
                .collect(Collectors.toSet());
        if (!dataSets.isEmpty() && (dataSets.contains(PersonType.REPV_LEGAL_PERSON)
                || dataSets.contains(PersonType.REPV_NATURAL_PERSON))) {
            return dataSets.size() == MAXIMUM_MDS_FOR_REPRESENTATION_CASE;
        } else {
            return true;
        }
    }
...
//        }
    ...
    public AuthnRequest marshallRequest(@Nonnull IEidasAuthenticationRequest eidasRequestToBeSent,
        ...
        eidasRequestToBeSent = new EidasAuthenticationRequest.Builder(eidasRequestToBeSent)
//                .requesterId(null)
//                .nameIdFormat(null)
                .build();
