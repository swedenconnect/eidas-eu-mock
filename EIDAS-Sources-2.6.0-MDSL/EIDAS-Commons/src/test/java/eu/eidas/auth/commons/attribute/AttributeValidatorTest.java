...
public class AttributeValidatorTest {
    ...
    /**
     * Test method for
     * {@link AttributeValidator#marshallAttribute(AttributeValue)}
     * when the {@link AttributeValueMarshaller#marshal(AttributeValue)} throws a AttributeValueMarshallingException
     * then marshallAttribute should throw InvalidParameterEIDASException
     * <p>
     * Must fail.
     */
    @Test
    public void testMarshallAttributeHandlesAttributeValueMarshallingException() throws AttributeValueMarshallingException {
        expectedException.expect(InvalidParameterEIDASException.class);
        expectedException.expectMessage("Invalid value for attribute FamilyName. AttributeValue could not be marshalled.");

        final AttributeValueMarshaller <String> attributeValueMarshaller = Mockito.mock(AttributeValueMarshaller.class);
        Mockito.doThrow(AttributeValueMarshallingException.class)
                .when(attributeValueMarshaller).marshal(any());

        final AttributeDefinition <String> attributeDefinition= new AttributeDefinition.Builder<String>()
                ...
                .build();
        final AttributeValidator validator = AttributeValidator.of(attributeDefinition);

        final AttributeValue mockAttributeValue = Mockito.mock(AttributeValue.class);
        Mockito.when(mockAttributeValue.getValue()).thenReturn("test1234567890");

        validator.validate(mockAttributeValue);
    }
}
