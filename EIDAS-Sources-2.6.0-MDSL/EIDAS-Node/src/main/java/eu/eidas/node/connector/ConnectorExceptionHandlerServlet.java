...
public final class ConnectorExceptionHandlerServlet extends AbstractNodeServlet {
    ...
    private void handleError(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!StringUtils.isBlank(exception.getSamlTokenFail()) && null != errorRedirectUrl) {
            redirectionErrorPage = NodeViewNames.SUBMIT_ERROR.toString();
        } else if (exception instanceof EidasNodeException || exception instanceof SecurityEIDASException) {
            redirectionErrorPage = NodeViewNames.PRESENT_ERROR.toString();
        } else {
            LOG.debug("BUSINESS EXCEPTION - null redirectUrl or SAML response");
            redirectionErrorPage = NodeViewNames.INTERCEPTOR_ERROR.toString();
        }

        //Forward to error page
        response.setStatus(HttpServletResponse.SC_OK);
        forwardRequest(redirectionErrorPage, request, response);
    }
