...
@WebFilter(filterName = "ProxyServiceOutgoingEidasResponseLoggerFilter",
        servletNames = {"ServiceExceptionHandler"},
...
public class ProxyServiceOutgoingEidasResponseLoggerFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        try {
            ...
            if (httpServletRequest.getRequestURI().endsWith("connectorRedirect.jsp") || containsSamlTokenFail(httpServletRequest)) {
                ...
                getProxyServiceOutgoingSamlResponseLoggerImpl().logFullMessage(httpServletRequest);
            }
    ...
    private boolean containsSamlTokenFail(HttpServletRequest httpServletRequest) {
        Object attribute = httpServletRequest.getAttribute("javax.servlet.error.exception");
        if (attribute instanceof  AbstractEIDASException && null != attribute) {
            final AbstractEIDASException abstractEIDASException = (AbstractEIDASException) attribute;
            final String samlTokenFail = abstractEIDASException.getSamlTokenFail();
            return null != samlTokenFail && StringUtils.isNotBlank(samlTokenFail);
        }
        return false;
    }
