...
import eu.eidas.auth.engine.LegacySha1DigestX509Certificate;
import eu.eidas.auth.engine.LegacySha1DigestX509Credential;
...
public abstract class AbstractProtocolEncrypter extends AbstractProtocolCipher implements ProtocolEncrypterI {
    ...
    protected AbstractProtocolEncrypter(@Nonnull EncryptionConfiguration encryptionConfiguration)
        ...
        Preconditions.checkNotEmpty(encryptionConfiguration.getEncryptionCertificates(), "encryptionCertificates");
    ...
    public Response encryptSamlResponse(@Nonnull Response authResponse, @Nonnull X509Certificate destinationCertificate,
        ...
        if (destinationCertificate instanceof LegacySha1DigestX509Certificate) {
            X509Certificate certificate = ((LegacySha1DigestX509Certificate) destinationCertificate).getCertificate();
            credential = CertificateUtil.toCredential(certificate);
            credential = new LegacySha1DigestX509Credential(credential);
        } else {
            credential = CertificateUtil.toCredential(destinationCertificate);
        }
