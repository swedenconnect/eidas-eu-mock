...
//        assertTrue(encryptModuleFileContent.contains("<entry key=\"response.encryption.mandatory\">true</entry>"));
    ...
    public void testConstructorAccessorIOException() throws ProtocolEngineConfigurationException, IOException {
        exception.expect(ProtocolEngineConfigurationException.class);
        exception.expectMessage("Error (no. 203005) processing request : invalid.samlengine.configuration.error");

        final SingletonAccessor<ImmutableMap<String, ProtocolEngineConfiguration>> singletonAccessor = Mockito.mock(SingletonAccessor.class);
    ...
    public void testCreateProtocolEngineAccessorIOException() throws ProtocolEngineConfigurationException, IOException {
        exception.expect(EIDASSAMLEngineRuntimeException.class);
        exception.expectCause(Is.isA(ProtocolEngineConfigurationException.class));
        exception.expectMessage("Error (no. 203005) processing request : invalid.samlengine.configuration.error");
    ...
    private ImmutableMap<String, ProtocolEngineConfiguration> getConfigurationMapAccessorInstance() throws IOException {
