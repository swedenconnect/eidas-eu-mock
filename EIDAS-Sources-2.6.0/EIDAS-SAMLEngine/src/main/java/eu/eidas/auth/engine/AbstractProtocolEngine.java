...
public abstract class AbstractProtocolEngine {
    ...
    public X509Certificate getSigningCertificate() throws EIDASSAMLEngineException {
        KeyStore.PrivateKeyEntry signatureKey = getSigner().getSignatureConfiguration().getSignatureKeyAndCertificate();
        return (X509Certificate) signatureKey.getCertificate();
    }
    ...
    private boolean isOnlySupportingOldProtocolVersion(String issuer) {
        List<EidasProtocolVersion> protocolVersionsList = getProtocolProcessor().getMetadataProtocolVersions(issuer);
        if (protocolVersionsList != null && !protocolVersionsList.isEmpty()) {
            EidasProtocolVersion issuerHighestProtocolVersion =
                    EidasProtocolVersion.getHighestProtocolVersion(protocolVersionsList);
            return EidasProtocolVersion.PROTOCOL_VERSION_1_1.isHigherProtocolVersion(issuerHighestProtocolVersion);
        }
        return false;
    }
    ...
    protected X509Certificate getEncryptionCertificate(@Nonnull String requestIssuer,
        ...
        if (StringUtils.isNotBlank(destinationCountryCode) && null != protocolEncrypter
            ...
            if (isOnlySupportingOldProtocolVersion(requestIssuer)) {
                if (encryptionCertificate != null && encryptionCertificate.getPublicKey() != null
                        && JCAConstants.KEY_ALGO_RSA.equalsIgnoreCase(encryptionCertificate.getPublicKey().getAlgorithm())) {
                    LegacySha1DigestX509Certificate encapsulatedEncryptionCertificate = new LegacySha1DigestX509Certificate(encryptionCertificate);
                    return encapsulatedEncryptionCertificate;
                }
            }
