...
public class SpecificProxyserviceCommunicationServiceImplTest {
     ...
     * Test method for
     * {@link SpecificProxyserviceCommunicationServiceImpl#getAndRemoveResponse(String, Collection)}
     * with a response with a invalid response status statusCode.
     * <p/>
     * Must throw a specificCommunicationException.
     */
    @Test
    public void testRemoveResponseFromCommunicationCacheInvalidResponseStatus() throws Exception {
        thrown.expect(SpecificCommunicationException.class);
        thrown.expectMessage("StatusCode : invalidStatusCode");

        final ILightResponse iLightResponseIn = LightResponseTestHelper
                .createLightResponse("inResponseTo", "invalidStatusCode", "subject", "urn:oasis:names:tc:SAML:1.1:nameid-format:unspecified");
        final BinaryLightToken binaryLightToken = getSpecificCommunicationService().putResponse(iLightResponseIn);
        final String tokenBase64 = BinaryLightTokenHelper.encodeBinaryLightTokenBase64(binaryLightToken);
        getSpecificCommunicationService().getAndRemoveResponse(tokenBase64,iLightResponseIn.getAttributes().getDefinitions());
    }
