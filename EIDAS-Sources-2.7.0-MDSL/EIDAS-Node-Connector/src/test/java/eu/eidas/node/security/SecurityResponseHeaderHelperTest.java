...
public class SecurityResponseHeaderHelperTest {
    ...
    private void setUpApplicationContext() throws NoSuchFieldException, IllegalAccessException {
    ...
    private void assertReportTo(String cspPolicies) {
        String reportURi = configurationSecurityBean.getCspReportingUri();
        if (StringUtils.isBlank(reportURi)) {
            assertFalse(cspPolicies.contains("report-to"));
        } else {
            assertTrue(cspPolicies.contains("report-to"));
        }
    }
