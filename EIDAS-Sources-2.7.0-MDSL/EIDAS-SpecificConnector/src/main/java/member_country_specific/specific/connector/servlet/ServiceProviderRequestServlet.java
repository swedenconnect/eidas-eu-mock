...
public class ServiceProviderRequestServlet extends AbstractSpecificConnectorServlet {
    ...
    private void execute(@Nonnull final HttpServletRequest httpServletRequest,
        ...
        final BinaryLightToken binaryLightToken = putRequestInCommunicationCache(new LightRequest.Builder(iLightRequest).build());
    ...
    private BinaryLightToken putRequestInCommunicationCache(ILightRequest iLightRequest) throws ServletException {
        ...
        try {
            final SpecificCommunicationService springManagedSpecificConnectorCommunicationService =
                    (SpecificCommunicationService) getApplicationContext().getBean(SpecificConnectorBeanNames.SPECIFIC_CONNECTOR_COMMUNICATION_SERVICE.toString());
