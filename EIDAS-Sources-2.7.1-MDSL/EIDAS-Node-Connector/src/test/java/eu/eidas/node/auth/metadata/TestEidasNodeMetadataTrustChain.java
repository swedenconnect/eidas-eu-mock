...
public class TestEidasNodeMetadataTrustChain {
     ...
     * {@link eu.eidas.auth.engine.core.impl.AbstractProtocolSigner#validateMetadataSignature(SignableXMLObject)}
     * when the certificates published in the metadata are the entire trust chain (metadatanode, intermediatecametadata, rootcametadata)
     * plus one additional one not belonging to the trust chain and
     * when that certificate is a trusted one by the {@link MetadataSignerI},
     * <p/>
     * Must fail.
     */
    @Test
    public void testValidMetadataSignatureExtraCertificateWhichDoesNotSignMetadataButTrusted() throws Exception {
        ...
        ProtocolEngineI protocolEngineI = getEngine("METADATA_WRONG_TRUST");
        MetadataSignerI checker = (MetadataSignerI) protocolEngineI.getSigner();
        java.security.cert.X509Certificate additionalX509CertificateJava = protocolEngineI.getSigner().getSignatureConfiguration().getTrustedCertificates().asList().get(1);
        X509Certificate additionalX509CertificateOpensaml = KeyInfoSupport.buildX509Certificate(additionalX509CertificateJava);
        entitiesDescriptor.getSignature().getKeyInfo().getX509Datas().get(0).getX509Certificates().add(additionalX509CertificateOpensaml);
     ...
     * Test method for
     * {@link eu.eidas.auth.engine.core.impl.AbstractProtocolSigner#validateMetadataSignature(SignableXMLObject)}
     * when the certificates published in the metadata are not the entire trust chain (metadatanode), intermediatecametadata and rootcametadata are not published, and
     ...
     */
    @Test
    public void testValidMetadataSignatureWithoutIntermediateCAWithoutRootCAPublishedTrustingIntermediateCARootCA() throws Exception {
