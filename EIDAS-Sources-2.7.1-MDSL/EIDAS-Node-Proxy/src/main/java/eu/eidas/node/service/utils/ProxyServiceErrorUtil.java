...
public class ProxyServiceErrorUtil {
     ...
     */
    public static void processSAMLEngineException(Exception exception, Logger logger, EidasErrorKey redirectError) {
        ...
        if (EidasErrorKey.DOC_TYPE_NOT_ALLOWED_CODE.toString().equals(errorCode)) {
            ...
            EidasErrorKey errorKey = EidasErrorKey.fromCode(errorCode);
            String message = EidasErrors.get(errorKey.errorMessage());
            String beanName = ProxyBeanNames.SYSADMIN_MESSAGE_RESOURCES.toString();
            ...
            final String errorMessage = msgResource.getMessage(message, new Object[]{
                    EidasErrors.get(errorKey.errorCode())}, Locale.getDefault());
