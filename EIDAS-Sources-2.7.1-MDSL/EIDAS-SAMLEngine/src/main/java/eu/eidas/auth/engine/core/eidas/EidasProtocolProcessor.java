...
public class EidasProtocolProcessor implements ProtocolProcessorI {
    ...
    private void throwValidationException(@Nonnull EidasErrorKey errorKey, @Nonnull String additionalInformation)
            throws EIDASSAMLEngineException {
        LOG.error(SAML_EXCHANGE, additionalInformation);
        throw new EIDASSAMLEngineException(
                EidasErrors.get(errorKey.errorCode()),
                EidasErrors.get(errorKey.errorMessage()), additionalInformation);
    }
        ...
        for (Map.Entry<AttributeDefinition<?>, ImmutableSet<? extends eu.eidas.auth.commons.attribute.AttributeValue<?>>> entry : requestedAttributes
            ...
            if (supportedAttributeNames.contains(fullName)) {
                ...
                throw new EIDASSAMLEngineException(
                        ...
                        EidasErrors.get(EidasErrorKey.EIDAS_MANDATORY_ATTRIBUTES.errorMessage()), additionalInformation);
            } else {
...
/*              String friendlyName = requestedAttribute.getFriendlyName();
    ...
    private IAuthenticationRequest validateRequestAgainstMetadata(@Nonnull IAuthenticationRequest request,
        ...
        if (updatedRequest.getRequestedAttributes().isEmpty()) {
            ...
            LOG.error(SAML_EXCHANGE, additionalInformation);
            throw new EIDASSAMLEngineException(
                    EidasErrors.get(EidasErrorKey.COLLEAGUE_REQ_ATTR_NULL.errorCode()),
                    EidasErrors.get(EidasErrorKey.COLLEAGUE_REQ_ATTR_NULL.errorMessage()), additionalInformation);
        }
     ...
     */
    private void processEIDASMetadataProviderException(Exception exception) throws EIDASSAMLEngineException {
