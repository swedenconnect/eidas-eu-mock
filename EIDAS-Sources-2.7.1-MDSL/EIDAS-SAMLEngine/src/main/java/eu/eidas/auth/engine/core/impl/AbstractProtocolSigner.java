...
public abstract class AbstractProtocolSigner implements ProtocolSignerI, MetadataSignerI {
    ...
    protected AbstractProtocolSigner(@Nonnull SignatureConfiguration signatureConfiguration)
        ...
        if (shouldUpdateDigestValues) {
            this.signatureConfiguration = new SignatureConfiguration(
                    signatureConfiguration.isCheckedValidityPeriod(),
    ...
    protected AbstractProtocolSigner(boolean checkedValidityPeriod,
            ...
            throws ProtocolEngineConfigurationException {
        this(new SignatureConfiguration(checkedValidityPeriod,
    ...
    public X509Credential getPublicMetadataSigningCredential() {
        KeyStore.PrivateKeyEntry privateKeyEntry = signatureConfiguration.getMetadataSigningKeyAndCertificate();
        X509Credential publicMetadataSigningCredential = CertificateUtil.createCredential(privateKeyEntry);
    ...
    public X509Credential getPublicSigningCredential() {
        KeyStore.PrivateKeyEntry privateKeyEntry = signatureConfiguration.getSignatureKeyAndCertificate();
        X509Credential publicSigningCredential = CertificateUtil.createCredential(privateKeyEntry);
    ...
    public <T extends SignableXMLObject> T sign(@Nonnull T signableObject, boolean onlyKeyInfoNoCert) throws EIDASSAMLEngineException {
        if (signableObject instanceof AuthnRequest) {
            SigningContext requestSigningContext = new SigningContext.Builder()
                    .setSigningCredential(this.requestSigningContext.getSigningCredential())
                    .setSigningAlgorithm(this.requestSigningContext.getSigningAlgorithm())
                    .setSignWithKeyValue(onlyKeyInfoNoCert)
                    .build();
            return sign(signableObject, requestSigningContext);
        } else {
            SigningContext responseSigningContext = new SigningContext.Builder()
                    .setSigningCredential(this.responseSigningContext.getSigningCredential())
                    .setSigningAlgorithm(this.responseSigningContext.getSigningAlgorithm())
                    .setSignWithKeyValue(onlyKeyInfoNoCert)
                    .build();
            return sign(signableObject, responseSigningContext);
        }
    ...
    public boolean isRequestSignWithKey() {
        return signatureConfiguration.isRequestSignWithKey();
    }

    @Override
    public boolean isResponseSignWithKey() {
        return signatureConfiguration.isResponseSignWithKey();
    }
