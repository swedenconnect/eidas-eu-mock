...
public class ProtocolEngine extends AbstractProtocolEngine implements ProtocolEngineI {
    ...
    private void validateAssertionSignatures(Response response) throws EIDASSAMLEngineException {
        try {
            ...
            if (validateSign) {
                final String proxyMetadataUrl = response.getIssuer().getValue();
                X509Certificate signatureCertificate = getProtocolProcessor().getMetadataParameters(proxyMetadataUrl)
                        .getResponseSignatureCertificate();
    ...
    private AuthnRequest validateSignature(final AuthnRequest request) throws EIDASSAMLEngineException {
        ...
        if (validateSign) {
            ...
            try {
                final String connectorMetadataUrl = issuer.getValue();
                X509Certificate signatureCertificate = getProtocolProcessor()
                        .getMetadataParameters(connectorMetadataUrl)
                        .getRequestSignatureCertificate();
    ...
    private Response validateSignatureAndDecryptAndValidateAssertionSignatures(Response response)
        ...
        checkKeyTransportAlgorithm(validResponse, issuer.getValue());
     ...
     */
    private void checkKeyTransportAlgorithm(Response response, String issuer) throws EIDASSAMLEngineException {
        try {
