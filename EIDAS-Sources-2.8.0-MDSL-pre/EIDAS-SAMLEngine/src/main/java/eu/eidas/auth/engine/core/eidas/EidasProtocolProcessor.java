...
public class EidasProtocolProcessor implements ProtocolProcessorI {
    ...
    public boolean checkMandatoryAttributes(@Nullable ImmutableAttributeMap immutableAttributeMap) {
        ...
        return !(legalPersonIncompleteMinimumDataSet || naturalPersonIncompleteMinimumDataSet) &&
...
//        }
    ...
    public String getCountryCode(SignableSAMLObject signableSAMLObject) throws EIDASSAMLEngineException {
        return getMetadataCountryCode(signableSAMLObject)
                .orElse(getCountryCodeFromSignatureCertificate(signableSAMLObject)); // backward compatibility with versions older than 2.5
    }
		...
		if(signature != null){
    		if (CertificateUtil.isSignatureWithCertificate(signature)){
    		    try {
    		        ...
    		        throw new EIDASSAMLEngineException(EidasErrorKey.SAML_ENGINE_INVALID_CERTIFICATE, "Failed to extract country from cert", e);
                }
    		}else{
    			if (signableSAMLObject instanceof RequestAbstractType){
	    			...
	    			originCountryCode = CertificateUtil.getCountry(
                            getMetadataParameters(issuer.getValue()).getRequestSignatureCertificate());
    			}else if (signableSAMLObject instanceof Response){
    				Issuer issuer = ((Response)signableSAMLObject).getIssuer();
	    			originCountryCode = CertificateUtil.getCountry(
                            getMetadataParameters(issuer.getValue()).getResponseSignatureCertificate());
    			}
    		}
        }
    ...
    /**
     * @deprecated use {@link EidasMetadataParametersI#getNodeCountry()} from {@link #getMetadataParameters(String)} instead
     */
    @Deprecated
    @Override
    public Optional<String> getMetadataNodeCountryCode(@Nullable String metadataUrl) {
        return getMetadataParametersOrEmpty(metadataUrl).map(EidasMetadataParametersI::getNodeCountry);
    }

    /**
     * @deprecated use {@link HighLevelMetadataParamsI#getMetadataProtocolVersions()} from {@link #getMetadataParameters(String)} instead
     */
    @Deprecated
    @Override
    public List<EidasProtocolVersion> getMetadataProtocolVersions(@Nullable String metadataUrl) {
        return getMetadataParametersOrEmpty(metadataUrl)
                .map(HighLevelMetadataParamsI::getMetadataProtocolVersions)
    ...
    public HighLevelMetadataParamsI getMetadataParameters(String metadataUrl) throws EIDASSAMLEngineException {
        try {
            ...
            final EidasMetadataParametersI eidasMetadata = metadataFetcher.getEidasMetadata(metadataUrl, metadataSigner, metadataClock);
    ...
    public X509Certificate getEncryptionCertificate(@Nullable String requestIssuer) throws EIDASSAMLEngineException {
        return getMetadataParameters(requestIssuer).getEncryptionCertificate();
    }
    ...
    /**
     * @deprecated use {@link HighLevelMetadataParamsI} from {@link #getMetadataParameters(String)} instead
     */
    @Deprecated
    @Nullable
    public MetadataEncryptionHelper getMetadataEncryptionHelper() {
        if (null == metadataFetcher || null == metadataSigner || null == metadataClock) {
            ...
            return new MetadataEncryptionHelper(metadataFetcher, metadataSigner, metadataClock);
        }
    }
    ...
    public MetadataSignatureHelper getMetadataSignatureHelper() {
        if (null == metadataFetcher || null == metadataSigner || null == metadataClock) {
            ...
            return new MetadataSignatureHelper(metadataFetcher, metadataSigner, metadataClock);
        }
    }
    ...
    public X509Certificate getRequestSignatureCertificate(@Nonnull String issuer) throws EIDASSAMLEngineException {
        return getMetadataParameters(issuer).getRequestSignatureCertificate();
    }
    ...
    public X509Certificate getResponseSignatureCertificate(@Nonnull String issuer) throws EIDASSAMLEngineException {
        return getMetadataParameters(issuer).getResponseSignatureCertificate();
    }
     ...
     * @throws EIDASSAMLEngineException in case of errors
     * @since 1.1
     * @deprecated use {@link HighLevelMetadataParamsI#getSupportedAttributes()} from {@link #getMetadataParameters(String)} instead
     */

    @Deprecated
    @Nonnull
    @SuppressWarnings("squid:S2583")
    public Set<String> getSupportedAttributes(@Nonnull String issuer) throws EIDASSAMLEngineException {
        return Optional.of(getMetadataParameters(issuer).getIDPRoleDescriptor())
                .map(EidasMetadataRoleParametersI::getSupportedAttributes)
