...
import eu.eidas.security.AbstractNodeServlet;
...
public class ContentSecurityPolicyReportServlet extends AbstractNodeServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        String cspHeader = response.getHeader(SecurityResponseHeaderHelper.CONTENT_SECURITY_POLICY_HEADER);
        ...
        try (BufferedReader reader = request.getReader()) {
            if ((read = reader.read(buffer, 0, MAX_SIZE)) != -1) {
                ...
                while ((line = newReader.readLine()) != null) {
                    String stringToLog = "BUSINESS EXCEPTION : Content security violation : " + line;
                    LOG.info(LoggingMarkerMDC.SECURITY_WARNING,stringToLog);
                }
            }
        } catch (IOException e) {
