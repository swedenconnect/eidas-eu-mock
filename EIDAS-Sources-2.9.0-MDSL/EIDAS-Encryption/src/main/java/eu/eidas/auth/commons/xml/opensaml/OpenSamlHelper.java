...
import eu.eidas.encryption.config.EidasJCENameConfiguration;
...
public final class OpenSamlHelper {
    ...
    static {
        ...
        try {
            ...
            try(
                InputStream xmldsigCorSchemaXsd = getResourceAsStream(classLoader,"xmldsig-core-schema.xsd");

                InputStream samlEidasExtensionXsd = getResourceAsStream(classLoader,"eidas/saml_eidas_extension.xsd");

                InputStream samlEidasLegalPersonXsd = getResourceAsStream(classLoader,"eidas/saml_eidas_legal_person.xsd");
                InputStream samlEidasNaturalPersonXsd = getResourceAsStream(classLoader,"eidas/saml_eidas_natural_person.xsd");
                InputStream samlEidasRepresentativeLegalPersonXsd = getResourceAsStream(classLoader,"eidas/saml_eidas_representative_legal_person.xsd");
                InputStream samlEidasRepresentativeNaturalPersonXsd = getResourceAsStream(classLoader,"eidas/saml_eidas_representative_natural_person.xsd")
            ){
                scBuilder.addSchema(xmldsigCorSchemaXsd);

                scBuilder.addSchema(samlEidasExtensionXsd);

                scBuilder.addSchema(samlEidasLegalPersonXsd);
                scBuilder.addSchema(samlEidasNaturalPersonXsd);
                scBuilder.addSchema(samlEidasRepresentativeLegalPersonXsd);
                scBuilder.addSchema(samlEidasRepresentativeNaturalPersonXsd);
    ...
    /**
     * Pre-Initialize EidasDefaultSecurityConfiguration
     * Setup a secured parser pool.
     */
    ...
    public static Element marshallToDom(@Nonnull XMLObject xmlObject) throws MarshallException {
        ...
        } catch (ParserConfigurationException | MarshallingException ex) {
            LOG.error("Marshall exception for " + xmlObject + ": " + ex, ex);
            throw new MarshallException(ex);
        }
    }
    ...
    private static InputStream getResourceAsStream(ClassLoader classLoader, String name) {
        ...
        if (null == resourceAsStream) {
            throw new IllegalStateException("Unable to load '" + name + "' from classLoader: " + classLoader);
        }
    ...
    private static ClassLoader getClassLoader(){
        ...
        if(classLoader == null){
            classLoader = OpenSamlHelper.class.getClassLoader();
        }
        if(classLoader == null){
