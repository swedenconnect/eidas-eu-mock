...
public class AbstractCachingMetadataFetcherTest {
    ...
    public void getEidasMetadataFetchNewMetadata() throws EIDASMetadataException {
        ...
        final IMetadataCachingService iMetadataCachingService = mock(IMetadataCachingService.class);
        doReturn(mock(EidasMetadataParametersI.class)).when(iMetadataCachingService).getEidasMetadataParameters(anyString());
        doReturn(iMetadataCachingService).when(spyCachingMetadataFetcher).getCache(any(), any());
    ...
    public void getEidasMetadataNoValidNewMetadata() throws EIDASMetadataException {
        ...
        final IMetadataCachingService iMetadataCachingService = mock(IMetadataCachingService.class);
        final EidasMetadataParametersI cachedMetadata = mock(EidasMetadataParametersI.class);
        Mockito.when(cachedMetadata.getValidUntil()).thenReturn(ZonedDateTime.now(ZoneOffset.UTC).minusHours(2)); //expired
        Mockito.when(mockMetadataClock.getCurrentTime()).thenReturn(ZonedDateTime.now(ZoneOffset.UTC));
        doReturn(cachedMetadata).when(iMetadataCachingService).getEidasMetadataParameters(anyString());
        doReturn(new ValidityCheckingMetadataCachingService(mockMetadataClock, iMetadataCachingService)).when(spyCachingMetadataFetcher).getCache(any(), any());
        doReturn(mock(EntityDescriptor.class)).when(spyCachingMetadataFetcher).fetchEntityDescriptor(anyString());

        final EidasMetadataParametersI eidasMetadata = spyCachingMetadataFetcher.getEidasMetadata("https://url.be", mockMetadataSigner, mockMetadataClock);
    }
}