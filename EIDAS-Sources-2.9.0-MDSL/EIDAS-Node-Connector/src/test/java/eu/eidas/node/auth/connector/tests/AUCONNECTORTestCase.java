...
public class AUCONNECTORTestCase {
    ...
    public void testGetAuthenticationResponseWithNoAttributes() {
        expectedException.expect(ConnectorError.class);
        expectedException.expectMessage(EidasErrors.get(EidasErrorKey.EIDAS_MANDATORY_ATTRIBUTES.errorMessage()));
    ...
    /**
     * Test method for
     * {@link AUCONNECTOR#validateLightRequestDatasetLimit(ILightRequest)}
     * when the attribute map contains attribute from mutual exclusive Natural and Legal datasets
     * <p>
     * Must fail.
     */
    @Test
    public void testRequestNaturalAndLegalDatasets() {
        expectedException.expect(allOf(
                isA(ConnectorError.class),
                hasProperty("message", containsString(EidasErrors.get(EidasErrorKey.SPROVIDER_SELECTOR_INVALID_ATTR.errorMessage()))),
                hasProperty("additionalInformation", containsString("Requesting both Natural and Legal datasets is not allowed"))
        ));
    ...
    private ImmutableAttributeMap setUpMockRequestedAttributes(String type){
        ...
        final Set<AttributeDefinition<?>> mockAttributeDefinitions;
        if ("both".equals(type)){
            mockAttributeDefinitions = Set.of(
            ...
            mockAttributeDefinitions = Set.of(mockAttributeDefinition);
