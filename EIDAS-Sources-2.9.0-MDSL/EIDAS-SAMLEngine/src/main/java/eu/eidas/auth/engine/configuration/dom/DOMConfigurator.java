...
public final class DOMConfigurator {
    ...
    private static final String DEFAULT_SAML_PROPERTIES_XML = "default/defaultSamlEngineProperties.xml";
    ...
    private static SamlEngineClock configureClock(@Nonnull String instanceName, @Nonnull InstanceEntry instanceEntry)
        ...
        try {
            ...
            throw new ProtocolEngineConfigurationException(EidasErrorKey.SAML_ENGINE_CONFIGURATION_ERROR,
                    "Error creating clock for SAML engine \"" + instanceName + "\" in " + clockClassName + " due to " , e);
        }
    }
    ...
    private static ProtocolProcessorI configureProtocolProcessor(@Nonnull String instanceName,
                                                                 ...
                                                                 @Nullable ProtocolSignerI protocolSigner)
            throws ProtocolEngineConfigurationException {
        ...
        try {
            ...
            Map<String, String> parameters = protocolProcessorConfigurationEntry.getParameters();
            ...
            MetadataFetcherI metadataFetcher = configureMetadataFetcher(instanceName, defaultPath, metadataFetcherClassName);
            ...
            if (null != protocolProcessor) {
                protocolProcessor.configure(protocolSigner);
            ...
            throw new ProtocolEngineConfigurationException(EidasErrorKey.SAML_ENGINE_CONFIGURATION_ERROR,
                    "Error creating protocol processor for SAML engine \"" + instanceName + "\" in "
                            + protocolProcessorClassName + " due to ", e);
        }
    }

    private static MetadataFetcherI configureMetadataFetcher(@Nonnull String instanceName, @Nullable String defaultPath,
                                                             String metadataFetcherClassName)
        ...
        if (StringUtils.isNotBlank(metadataFetcherClassName)) {
            ...
            Properties properties = (new ReloadableProperties(fileName, defaultPath)).getProperties();

            String flagWhitelist = (String) properties.get(ParameterKey.METADATA_FETCHER_WHITELIST_FLAG.getKey());
            String whitelist = (String) properties.get(ParameterKey.METADATA_FETCHER_WHITELIST.getKey());
    ...
    private static SamlEngineCoreProperties configureSamlEngineCore(@Nonnull String instanceName,
        ...
        if (null == samlEngineConfigurationEntry) {
        ...
        try {
            ...
            throw new ProtocolEngineConfigurationException(
                    EidasErrorKey.SAML_ENGINE_CONFIGURATION_ERROR, " : " +
                            "ConfigurationEntry: \"" + ConfigurationKey.SAML_ENGINE_CONFIGURATION.getKey()
    ...
    private static ProtocolSignerI configureSignature(@Nonnull String instanceName,
        ...
        try {
            ...
            LOG.error("Error creating signature for SAML engine \"" + instanceName + "\" in " + signerClassName
                    + " due to " + e, e);
            throw new ProtocolEngineConfigurationException(EidasErrorKey.SAML_ENGINE_CONFIGURATION_ERROR,
                    "Error creating signature for SAML engine \"" + instanceName + "\" in " + signerClassName
                            + " due to ", e);
        }
    }
    ...
    private static <T> T createReloadableProxyIfNeeded(@Nonnull final String instanceName,
        ...
        if (StringUtils.isBlank(overrideFile)) {
            return createReloadableProxyIfNeeded(instanceName, configurationEntry, configurationKey, defaultPath, type, constructor,
                                                 contextClassLoader, Map.<String, String>of());
        } else {
    ...
    private static <T> T createReloadableProxyIfNeeded(@Nonnull String instanceName,
                                                       ...
                                                       @Nonnull final Map<String, String> overrideParameters)
            throws IllegalAccessException, InvocationTargetException, InstantiationException {
        String fileConfiguration = configurationEntry.get(ParameterKey.FILE_CONFIGURATION);
        Map<String, String> staticParameters = configurationEntry.getParameters();

        Map<String, String> defaultParameters = getDefaultSamlProperties();
        ...
        if (StringUtils.isNotBlank(fileConfiguration)) {
            final SingletonAccessor<T> accessor =
                    ExternalConfigurationFileAccessor.newAccessor(instanceName, configurationKey.getKey(),
                                                                  ...
                                                                  defaultParameters, new MapConverter<T>() {
            ...
            if (constructor.getParameterTypes().length == 1) {
                return constructor.newInstance(Map.copyOf(allParameters));
            } else {
                return constructor.newInstance(Map.copyOf(allParameters),defaultPath);
            }
        }
    }

    private static Map<String, String> getDefaultSamlProperties() {
        final Properties defaultProperties = PropertiesLoader
                .loadPropertiesXMLFile(DEFAULT_SAML_PROPERTIES_XML);

        final Map<String, String> defaultPropertiesMap = new LinkedHashMap<>();
        for (final String propertyName : defaultProperties.stringPropertyNames()) {
            final String propertyValue = defaultProperties.getProperty(propertyName);
            defaultPropertiesMap.put(propertyName, propertyValue);
        }

        return Map.copyOf(defaultPropertiesMap);
    }
    ...
    public static ProtocolEngineConfiguration getProtocolConfiguration(@Nonnull String instanceName,
        ...
        ProtocolProcessorI protocolProcessor = configureProtocolProcessor(instanceName, defaultPath, instanceEntry, metadataSigner, (MetadataClockI) clock, signer);
     ...
     * @return a {@link Map} collection that contains {@link String} as
     ...
     */
    @Nonnull
    public static Map<String, ProtocolEngineConfiguration> getProtocolConfigurationMap(
    ...
    public static Map<String, ProtocolEngineConfiguration> getProtocolConfigurationMap(
        ...
        Map<String, ProtocolEngineConfiguration> configurationMap = new LinkedHashMap<>();
        for (final Map.Entry<String, InstanceEntry> entry : instanceMap.getInstances().entrySet()) {
            ...
            configurationMap.put(instanceName, configuration);
        }
    ...
    private static <T> T throwConfigurationException(@Nonnull String additionalInformation) throws ProtocolEngineConfigurationException {
        ...
        throw new ProtocolEngineConfigurationException(EidasErrorKey.SAML_ENGINE_CONFIGURATION_ERROR, additionalInformation);
    }
