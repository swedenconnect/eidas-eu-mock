...
public class LightJAXBCodecTest {
	...
	private static final ImmutableAttributeMap ATTRIBUTES = new ImmutableAttributeMap.Builder()
			...
			.put(EidasSpec.Definitions.CURRENT_GIVEN_NAME, new StringAttributeValue("Jean-Claude"),
					new StringAttributeValue("Jean"), new StringAttributeValue("Claude"))
			.put(EidasSpec.Definitions.BIRTH_NAME, new StringAttributeValue("Juncker"))
			.put(EidasSpec.Definitions.D_2012_17_EU_IDENTIFIER, new StringAttributeValue("Juncker12345"))
			.put(EidasSpec.Definitions.DATE_OF_BIRTH, new DateTimeAttributeValue(LocalDate.now()))
			.put(EidasSpec.Definitions.EORI, new StringAttributeValue("Juncker-eori"))
	...
	private static final Set<AttributeDefinition<?>> REGISTRY = ATTRIBUTES.getDefinitions();
...
//				result.getAttributes().getAttributeMap().entrySet());
		for (AttributeDefinition<?> d : lightResponse.getAttributes().getAttributeMap().keySet()) {
			Set<?> expectedSet = lightResponse.getAttributes().getAttributeValues(d);
			Set<?> actualSet = result.getAttributes().getAttributeValues(d);
