...
public class SpecificConnector {
    ...
    private ImmutableAttributeMap translateToEidasAttributes(@Nonnull final AuthenticationRequest specificRequest) {
        ...
        for (Attribute attribute : attributes) {
            ...
            final SortedSet<AttributeDefinition<?>> byFriendlyName = getCoreAttributeRegistry().getByFriendlyName(name);
            if (!byFriendlyName.isEmpty()) {
                ...
                if (attribute instanceof StringAttribute) {
                    ...
                    final AttributeValue dateAttributeValue = new DateTimeAttributeValue(LocalDate.ofInstant(value.toInstant(), ZoneId.systemDefault()));
    ...
    private List<Attribute> translateLightRequestAttributes(@Nonnull final ILightResponse lightResponse) {
        ...
        final Map<AttributeDefinition<?>, Set<? extends AttributeValue<?>>> attributeMap = lightResponseAttributes.getAttributeMap();

        final List<Attribute> attributes = new ArrayList<>();
        for (Map.Entry<AttributeDefinition<?>, Set<? extends AttributeValue<?>>> entry : attributeMap.entrySet()) {
            AttributeDefinition attributeDefinition = entry.getKey();
            final String friendlyName = attributeDefinition.getFriendlyName();

            List<? extends AttributeValue<?>> attributeValues = new ArrayList<>(entry.getValue());
    ...
    private DateAttribute translateDateAttribute(@Nonnull final String friendlyName, List<? extends AttributeValue<?>> attributeValues) {
        ...
        if (attributeValues.size() != 0) {
            ...
            final LocalDate value = (LocalDate) firstAttributeValue.getValue();
            Date dateValue = Date.from(value.atStartOfDay(ZoneId.systemDefault()).toInstant());
    ...
    private AddressAttribute translateAddressAttribute(List<? extends AttributeValue<?>> attributeValues, String friendlyName) {
