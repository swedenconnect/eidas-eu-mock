...
public final class CertificateUtil {
    ...
    public static void checkChainTrust(final X509Credential x509Credential,
        ...
        } catch (final CertificateRevokedException | MissingCertificateRevocationDataException |
                       UntrustedCertificateException ex) {
        ...
        try {
            ...
            if (!publicKeys.isEmpty()) {
                ...
                final Set<X509Certificate> publicKeyMatchingCertificates = availableCertificates.stream()
                        .filter(x509Certificate -> arePublicKeysEqual(x509Certificate.getPublicKey(), messagePublicKey))
                        .collect(Collectors.toSet());

                if (publicKeyMatchingCertificates.size() > 1) {
                    final List<String> matchingCertificateSerialNumbers = new ArrayList<>();
                    for(X509Certificate certificate: publicKeyMatchingCertificates) {
                        matchingCertificateSerialNumbers.add(String.valueOf(certificate.getSerialNumber()));
                    }
                    throw new IllegalStateException(
                            "Metadata contains multiple certificates for the saml message public key, no unique match. " +
                                    "Duplicate public keys detected for certificates with serial numbers:" + String.join(", ", matchingCertificateSerialNumbers));
                }
    ...
    private static boolean arePublicKeysEqual(PublicKey publicKey1, PublicKey publicKey2) {
        return publicKey1.equals(publicKey2) || Arrays.equals(publicKey1.getEncoded(), publicKey2.getEncoded());
    }
}
