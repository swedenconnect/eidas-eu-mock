...
public final class SAMLAuthnResponseEncrypter {
    ...
    /**
     * Encrypts the given SAML {@link Response} using the provided encryption credentials.
     * <p>
     * If TRACE logging is enabled, the response is marshalled and logged before encryption.
     * To avoid issues caused by stale or partial DOM structures, the response's DOM and child DOM
     * are explicitly released immediately after logging. This ensures the encryption operates on a clean state.
     *
     * @param samlResponse               the SAML response to encrypt
     * @param credential                 the encryption credential
     * @param isKeyInfoDisplayedAsRSAKey flag indicating if key info should be displayed as RSA key
     * @return the encrypted SAML response
     * @throws EncryptionException in case of marshalling or encryption errors
     */
    public Response encryptSAMLResponse(final Response samlResponse, final Credential credential, boolean isKeyInfoDisplayedAsRSAKey)
        ...
        try {
            if (LOGGER.isTraceEnabled()) {
                ...
                samlResponse.releaseDOM();
                samlResponse.releaseChildrenDOM(true);
            }
