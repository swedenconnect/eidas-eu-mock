...
public class Pkcs11Decrypter extends Decrypter {
    ...
    /**
     * Decrypts an encrypted key using the provided algorithm and KEK.
     * If the KEK is a supported PKCS#11 private key, uses custom decryption;
     ...
     * @throws IllegalArgumentException If the KEK is null.
     * @throws DecryptionException      If algorithm is missing or decryption fails.
     */
    @Nonnull
    ...
    public Key decryptKey(@Nonnull EncryptedKey encryptedKey, @Nonnull String algorithm, @Nonnull Key kek) throws DecryptionException {
        ...
        } else if (null == algorithm || algorithm.isEmpty()) {
    ...
    private byte[] unpadWithOaepMgf1(EncryptionMethod encMethod, byte[] dataDecryptionKeyOaepMGF1PaddedBytes) throws InvalidKeyException, InvalidAlgorithmParameterException, BadPaddingException {
        ...
        RSAPadding padding = RSAPadding.getInstance(RSAPadding.PAD_OAEP_MGF1, dataDecryptionKeyOaepMGF1PaddedBytes.length, JCAUtil.getSecureRandom(), parameterSpec);
    ...
    /**
     * Checks if the key is a supported PKCS#11 private key type
     * based on its class name.
     *
     * @param kek The key to check.
     * @return {@code true} if it’s a recognized PKCS#11 private key.
     */
    private boolean isPkcs11PrivateKey(Key kek) {
        String className = kek.getClass().getName();
        return "sun.security.pkcs11.P11Key$P11PrivateKey".equals(className) ||
                "sun.security.pkcs11.P11Key$P11RSAPrivateKeyInternal".equals(className);
    }
