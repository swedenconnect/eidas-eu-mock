...
import eu.eidas.ignite.IgniteNodeCacheManager;
...
public class IgniteCreatedExpirePolicyTest {
    ...
    private static IgniteNodeCacheManager concurrentCacheServiceIgniteNode;
     ...
     * and creates and configures a {@link eu.eidas.ignite.IgniteNodeCacheManager} instance.
     *
     * @throws IOException if {@code CONFIG_FILE_NAME} is not found.
     */
    @BeforeClass
    public static void initializeIgniteInstance() throws IOException {
        ...
        concurrentCacheServiceIgniteNode = new IgniteNodeCacheManager(
                igniteInstanceInitializerNode,
                Map.of(DEMO_CACHE_NAME, DEMO_CACHE_NAME)
        );
    }
    ...
    public static void stopIgnite() {
        String instanceName = (igniteInstanceInitializerNode.getInstance()).name();
    ...
    public void getConfiguredCacheWithExpiryPolicy() {
        ...
        Cache<String, String> configuredCache = concurrentCacheServiceIgniteNode.getCache(DEMO_CACHE_NAME);
        ...
        Assert.assertEquals(2, expiryPolicy.getExpiryForCreation().getDurationAmount());
        Assert.assertEquals(TimeUnit.SECONDS, expiryPolicy.getExpiryForCreation().getTimeUnit());
        Assert.assertTrue(expiryPolicy.getExpiryForCreation().equals(new Duration(TimeUnit.MILLISECONDS, 2000)));
    }
