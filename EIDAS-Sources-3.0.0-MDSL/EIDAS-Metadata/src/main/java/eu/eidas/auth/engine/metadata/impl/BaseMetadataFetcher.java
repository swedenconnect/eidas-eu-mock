...
public abstract class BaseMetadataFetcher implements MetadataFetcherI {
    ...
    protected EntityDescriptor fetchEntityDescriptor(@Nonnull String url) throws EIDASMetadataProviderException {
        ...
        try {
            ...
            if (metadata instanceof EntityDescriptor) {
                ...
                entityDescriptor = provider.getEntityDescriptor();
            }
            ...
            if (sample != null) {
                recordMetadataFetchTimer(sample, meterRegistry, url, resultStatus);
            }
        }
    ...
    /**
     * Records the duration of a metadata fetch operation in Micrometer using a {@link Timer}.
     * The metric is registered under a fixed name, with tags for the URL and status.
     *
     * @param sample   Micrometer {@link Timer.Sample} started before the fetch
     * @param registry Micrometer {@link MeterRegistry} where the timer should be recorded
     * @param url      The metadata URL, used as a metric tag
     * @param status   Outcome of the fetch (e.g., "success", "error", "exception")
     */
    private void recordMetadataFetchTimer(@Nullable Timer.Sample sample, @Nullable MeterRegistry registry, String url, String status) {
        if (null == sample || null == registry) {
            return;
        }

        final String timerName = "httpclient.metadata.fetch";
        Timer timer = Timer.builder(timerName)
                .description("Time taken to fetch SAML metadata")
                .tag("url", url)
                .tag("status", status)
                .register(registry);
