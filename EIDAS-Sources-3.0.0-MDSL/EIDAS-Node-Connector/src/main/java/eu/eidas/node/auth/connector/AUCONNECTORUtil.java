...
public final class AUCONNECTORUtil extends AUNODEUtil {
    ...
    public AUCONNECTORUtil(final CacheManager cacheManager) {
        // Obtaining the anti-replay cache service provider defined in configuration and call it for setting up cache
        setAntiReplayCache(cacheManager.getCache("antiReplayConnector"));
    }
    ...
    /**
     * Loads the skew time of a ProxyService, with the given service ID, from the properties file.
     *
     * @param serviceId the ID of the ProxyService.
     * @param skewType  the type of skew time (before or after).
     * @return the skew time in milliseconds as a {@link Long}, or {@code 0} if no skew time was found.
     */
