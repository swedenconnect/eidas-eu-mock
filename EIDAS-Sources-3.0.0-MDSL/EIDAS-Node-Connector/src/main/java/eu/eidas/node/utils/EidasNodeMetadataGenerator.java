...
public class EidasNodeMetadataGenerator {
        ...
        if (url != null && !url.isEmpty()) {
            try {
                ...
                if (EidasErrorKey.isErrorCode(eidasMetadataException.getErrorCode())) {
                    ConnectorErrorUtil.processMetadataException(eidasMetadataException, EidasErrorKey.SAML_ENGINE_NO_METADATA);
                }
    ...
    private ProtocolEngineI setSPMetadataRoleParams(final EidasMetadataParametersI emp, final EidasMetadataRoleParametersI emrp) throws EIDASSAMLEngineException {
        ...
        final List<X509Certificate> spEngineDecryptionCertificates = protocolEngine.getDecryptionCertificates();
        if (spEngineDecryptionCertificates != null) {
            emrp.setEncryptionCertificates(spEngineDecryptionCertificates);
        }
        emrp.setSigningCertificates(protocolEngine.getSigningCertificates());

        String nameIdFormatsString = getNodeProps().getProperty(EIDASValues.CONNECTOR_NAMEID_FORMATS.toString());
        List<String> nameIdFormatsList = EidasStringUtil.getDistinctValues(nameIdFormatsString);
        emrp.setNameIDFormats(new HashSet<>(nameIdFormatsList));
