...
import eu.eidas.auth.engine.metadata.impl.JCacheMetadataCachingService;
...
public class TestEidasNodeMetadataLoader {
    ...
    public void testgetEntityDescriptors() throws EIDASMetadataException {
        ...
        fetcher.setCache(simpleMetadataCaching(86400L));
    ...
    public void testValidateSignature() throws EIDASMetadataException {
        CachingMetadataFetcher fetcher = new CachingMetadataFetcher();
        FileMetadataLoader loader = new FileMetadataLoader();
        fetcher.setCache(simpleMetadataCaching(86400));
    ...
    private static IMetadataCachingService simpleMetadataCaching(long retention) {
        final ConcurrentMapJcacheServiceDefaultImpl cacheService = new ConcurrentMapJcacheServiceDefaultImpl();
        cacheService.setExpireAfterAccess(retention);
        final Cache cache = cacheService.getConfiguredCache();
        CacheManager cacheManager = mock(CacheManager.class);
        when(cacheManager.getCache(anyString())).thenReturn(cache);
        return new JCacheMetadataCachingService(cacheManager, "metadata");
    }
}
