...
public class AUSERVICESAML implements ISERVICESAMLService {
    ...
    public IResponseMessage processIdpSpecificResponse(IAuthenticationRequest originalRequest,
        ...
        try {
            ...
            LOG.info(LoggingMarkerMDC.SAML_EXCHANGE,
                    "ProxyService - Generating SAML Response to request with ID {}", originalRequest.getId());
            final HighLevelMetadataParamsI connectorMetadataParameters = engine.getProtocolProcessor()
                    .getMetadataParameters(originalRequest.getIssuer());
            String subjectNameIdFormat = response.getSubjectNameIdFormat();

            if (Boolean.parseBoolean(serviceUtil.getConfigs().getProperty("service.nameid.ifNeededRespondWithUnspecified"))) {
                if (!connectorMetadataParameters.getSPRoleDescriptor().getNameIDFormats().contains(response.getSubjectNameIdFormat())) {
                    subjectNameIdFormat = NameIDType.UNSPECIFIED;
                }
            }

            if (!getNameIdFormatList().contains(subjectNameIdFormat)) {
                throw new ProxyServiceError(
                        ...
                        "Proxy not configured for nameIdFormat " + subjectNameIdFormat);
            }
            AuthenticationResponse.Builder authnResponseBuilder = new AuthenticationResponse.Builder(response);
            authnResponseBuilder.subjectNameIdFormat(subjectNameIdFormat);
    ...
    public IAuthenticationRequest processConnectorRequest(String bindingFromHttp,
        ...
        try {
            ...
            final HighLevelMetadataParamsI issuerMetadataParameters = getIssuerMetadataParameters(engine, authnRequest.getIssuer());
            validateIssuerProtocolVersions(issuerMetadataParameters);
            final EidasMetadataRoleParametersI connectorRoleMetadata = issuerMetadataParameters.getSPRoleDescriptor();
            ...
            checkRequesterID(authnRequest, issuerMetadataParameters);
            ...
            validateRequestedAttributesForProtocol(authnRequest, issuerMetadataParameters);
            ...
            EidasErrorKey eidasErrorKey = e.getEidasErrorKey();
            if (Arrays.asList(
                    EidasErrorKey.INVALID_LOA_VALUE,
                    EidasErrorKey.COLLEAGUE_REQ_INVALID_LOA,
                    EidasErrorKey.EIDAS_REPRESENTATIVE_ATTRIBUTES,
                    EidasErrorKey.COLLEAGUE_REQ_INCONSISTENT_SPTYPE,
                    EidasErrorKey.COLLEAGUE_REQ_MISSING_SPTYPE).contains(eidasErrorKey)) {
                throw new ProxyServiceError(
                        EidasErrors.get(eidasErrorKey.errorCode()),
                        EidasErrors.get(eidasErrorKey.errorMessage()), e);
            }
    ...
    private void checkRequesterID(IAuthenticationRequest authnRequest, EidasMetadataParametersI issuerMetadataParameters) {
    ...
    @Nonnull
    private HighLevelMetadataParamsI getIssuerMetadataParameters(ProtocolEngineI engine, String issuer) throws EIDASSAMLEngineException {
        if (StringUtils.isNotBlank(issuer)) {
            try {
                return getSamlEngine().getProtocolProcessor().getMetadataParameters(issuer);
            } catch (EIDASSAMLEngineException e) {
                throw new ProxyServiceError(
    ...
    private void throwSamlFailureException(EidasErrorKey errorKey, IAuthenticationRequest authenticationRequest) {
        throw new SamlFailureResponseException(
                EidasErrors.get(errorKey.errorCode()),
                EidasErrors.get(errorKey.errorMessage()),
    ...
    private void checkNameIDFormat(IAuthenticationRequest authnRequest) {
        ...
        if (nameIdFormat != null) {
            Set<String> serviceNameIDFormats = new HashSet<>(getNameIdFormatList());
            if (!serviceNameIDFormats.contains(nameIdFormat)) {

                LOG.error("BUSINESS EXCEPTION : Invalid NameID format " + nameIdFormat);

                throw new SamlFailureResponseException(
                        EidasErrors.get(EidasErrorKey.COLLEAGUE_REQ_INVALID_NAMEID.errorCode()),
                        EidasErrors.get(EidasErrorKey.COLLEAGUE_REQ_INVALID_NAMEID.errorMessage()),
                        EIDASStatusCode.REQUESTER_URI.toString(),
    ...
    private void checkCountryCode(IAuthenticationRequest authnRequest) {
        ...
        if (isEmpty(countryCode) || !countryCode.equals(samlCountryCode)) {
            ...
            throw new SamlFailureResponseException(
                    EidasErrors.get(EidasErrorKey.COLLEAGUE_REQ_INVALID_COUNTRYCODE.errorCode()),
                    EidasErrors.get(EidasErrorKey.COLLEAGUE_REQ_INVALID_COUNTRYCODE.errorMessage()),
                    EIDASStatusCode.REQUESTER_URI.toString(),
                    authnRequest.getRelayState());
        }
    ...
    private void validateIssuerProtocolVersions(EidasMetadataParametersI connectorMetadataParameters) {
        ...
        if (!EidasProtocolVersion.isInteroperableWith(proxyProtocolVersions, destinationProtocolVersions)) {
            throw new ProxyServiceError(
                    EidasErrors.get(EidasErrorKey.PROTOCOL_VERSION_UNSUPPORTED.errorCode()),
                    EidasErrors.get(EidasErrorKey.PROTOCOL_VERSION_UNSUPPORTED.errorMessage()));
        }
    }

    /**
     * Validates the requested attributes for a given authentication request against the protocol version
     * specified in the connector metadata. Throws an exception if the protocol version is 1.4, higher, or unspecified,
     * and both natural and legal person attributes are present.
     *
     * @param authnRequest                The authentication request to validate.
     * @param connectorMetadataParameters The connector metadata parameters containing information about the
     *                                    protocol versions.
     */
    private void validateRequestedAttributesForProtocol(IAuthenticationRequest authnRequest, EidasMetadataParametersI connectorMetadataParameters) {
        ImmutableAttributeMap requestedAttributes = authnRequest.getRequestedAttributes();
        List<String> eidasProtocolVersions = connectorMetadataParameters.getEidasProtocolVersions();
        List<EidasProtocolVersion> protocolVersions = EidasProtocolVersion.fromString(eidasProtocolVersions);
        EidasProtocolVersion highestProtocolVersion = EidasProtocolVersion.getHighestProtocolVersion(protocolVersions);

        boolean hasNaturalPerson = requestedAttributes.getDefinitions().stream()
                .anyMatch(attributeDefinition -> attributeDefinition.getPersonType() == PersonType.NATURAL_PERSON);

        boolean hasLegalPerson = requestedAttributes.getDefinitions().stream()
                .anyMatch(attributeDefinition -> attributeDefinition.getPersonType() == PersonType.LEGAL_PERSON);

        if ((null == eidasProtocolVersions ||
                (highestProtocolVersion != EidasProtocolVersion.PROTOCOL_VERSION_1_2 &&
                        highestProtocolVersion != EidasProtocolVersion.PROTOCOL_VERSION_1_3)) &&
                hasNaturalPerson && hasLegalPerson) {
            throw new ProxyServiceError(
                    ...
                    "Both natural and legal person attributes cannot be present in the request " +
                            "for protocol version 1.4, higher or unspecified");
        }
    }
    ...
    public void setNameIdFormatList(String formats) {
        this.nameIdFormatList = EidasStringUtil.getDistinctValues(formats);
    }
}
