...
public class SamlResponseFailureServlet extends AbstractNodeServlet {
    ...
    public void doPost(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws ServletException, IOException {
        SamlFailureResponseException exception = (SamlFailureResponseException) httpServletRequest.getAttribute(RequestDispatcher.ERROR_EXCEPTION);
        ...
        try {
            if (null == storedRequest) {
                ...
                httpServletRequest.setAttribute(ProxySamlResponseViewMapping.RELAY_STATE, exception.getRelayState());
            }
            ...
            httpServletRequest.setAttribute(ProxySamlResponseViewMapping.SAML_TOKEN, samlTokenFail);
            httpServletRequest.setAttribute(ProxySamlResponseViewMapping.REDIRECT_URL, errorRedirectUrl);
            httpServletRequest.setAttribute(ProxySamlResponseViewMapping.EXCEPTION, exception);
            httpServletRequest.setAttribute(ProxySamlResponseViewMapping.BINDING, BindingMethod.POST.toString());

            httpServletResponse.setStatus(HttpServletResponse.SC_OK);
            forwardRequest(NodeViewNames.EIDAS_CONNECTOR_REDIRECT.toString(), httpServletRequest, httpServletResponse);

        } catch (EIDASSAMLEngineException e) {
            ProxyServiceError newException = new ProxyServiceError(e.getEidasErrorKey().errorCode(), e.getEidasErrorKey().errorMessage());
            httpServletRequest.setAttribute(RequestDispatcher.ERROR_EXCEPTION, newException);
