...
public final class CertificateValidator {
    ...
    public static final String CHECK_VALIDITY_PERIOD_PROPERTY = "check.certificate.validity.period";

    public static final String DISALLOW_SELF_SIGNED_CERTIFICATE_PROPERTY = "disallow.self.signed.certificate";
    ...
    public static void checkCertificateIssuer(X509Certificate certificate) throws EIDASSAMLEngineException {
        if (CertificateUtil.isCertificateSelfSigned(certificate)) {
            ...
            throw new EIDASSAMLEngineException(EidasErrorKey.INVALID_CERTIFICATE_SIGN,
                    "ERROR : The certificate with reference '" + certificate.getIssuerDN() + "' failed check (selfsigned)");
        }
    }
    ...
    public static void checkCertificateValidityPeriod(X509Certificate certificate) throws EIDASSAMLEngineException {
        ...
        if (currentDate.before(notBefore) || currentDate.after(notAfter)) {
            ...
            throw new EIDASSAMLEngineException(EidasErrorKey.INVALID_CERTIFICATE_SIGN,
                    "ERROR : The certificate with reference '"+ certificate.getIssuerDN() +
                            "' failed check (out of date) [notBefore=" + notBefore + ", notAfter=" + notAfter + "]");
        }
    }
