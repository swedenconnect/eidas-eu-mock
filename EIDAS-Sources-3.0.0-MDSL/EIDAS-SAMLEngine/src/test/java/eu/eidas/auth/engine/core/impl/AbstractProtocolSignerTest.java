...
public class AbstractProtocolSignerTest {
    ...
    /**
     * Test method for
     * {@link AbstractProtocolSigner#getTrustedCertificate(Signature, List, CertificateVerifierParams)}
     * when List is empty
     * <p>
     * Must fail
     */
    ...
    /**
     * Test method for
     * {@link AbstractProtocolSigner#getTrustedCertificateFromKeyValue(Signature, List)}
     * when a matching certificate is found in the trust store for the given {@link Signature}.
     * <p>
     * Must succeed.
     */
    @Test
    public void testGetTrustedCertificateFromKeyValue() throws Exception {
        final Signature mockSignature = Mockito.mock(Signature.class);
        ...
        final X509Credential trustedCredential = Mockito.mock(X509Credential.class);
        Mockito.when(trustedCredential.getEntityCertificate()).thenReturn(trustedCert);

        try (MockedStatic<CertificateUtil> mockedCertificateUtil = Mockito.mockStatic(CertificateUtil.class)) {
            mockedCertificateUtil.when(() ->
                    CertificateUtil.getMatchingCertificate(Mockito.eq(mockKeyInfo), Mockito.anyList())
            ).thenReturn(trustedCert);

            final List<Credential> trustedCredentialList = List.of(trustedCredential);

            final Method getTrustedCertificateFromKeyValueMethod = getDeclaredMethod(
                    "getTrustedCertificateFromKeyValue", Signature.class, List.class);
            final X509Credential result = (X509Credential) getTrustedCertificateFromKeyValueMethod.invoke(mockSigner, mockSignature, trustedCredentialList);

            Assert.assertNotNull("Expected a matching credential to be returned", result);
            Assert.assertEquals("Returned credential should match the trusted credential", trustedCredential, result);
        }
    }
     ...
     * Test method for
     * {@link AbstractProtocolSigner#getTrustedCertificateFromKeyValue(Signature, List)}
     * when a {@link CertificateException} is thrown during the matching process.
     * <p>
     * Must fail.
     */
    @Test
    public void testGetTrustedCertificateFromKeyValueWhenEIDASSAMLEngineExceptionIsThrown() throws Exception {
        expectedException.expect(EIDASSAMLEngineException.class);
        expectedException.expectMessage("Failed to match signature keyInfo to a known Trusted Credential");
        ...
        final X509Credential trustedCredential = Mockito.mock(X509Credential.class);
        Mockito.when(trustedCredential.getEntityCertificate()).thenReturn(Mockito.mock(X509Certificate.class));
        final List<Credential> trustedCredentialList = List.of(trustedCredential);

        try (MockedStatic<CertificateUtil> mockedCertificateUtil = Mockito.mockStatic(CertificateUtil.class)) {
            mockedCertificateUtil.when(() ->
                    CertificateUtil.getMatchingCertificate(Mockito.eq(mockKeyInfo), Mockito.anyList())
            ).thenThrow(new CertificateException("Mocked certificate exception"));

            final Method getTrustedCertificateFromKeyValueMethod = getDeclaredMethod("getTrustedCertificateFromKeyValue", Signature.class, List.class);
            try {
                getTrustedCertificateFromKeyValueMethod.invoke(mockSigner, mockSignature, trustedCredentialList);
     ...
     */
    @Test
    public void testCheckMetadataTrustAnchorWithEmptyTrustChain() {
    ...
    private void testInvalidCertificateSignatureHash(X509Certificate certificate) {
        try {
            Method validateCertificateSignatureHashMethod = getDeclaredMethod("validateCertificateSignatureHash", X509Certificate.class);
            validateCertificateSignatureHashMethod.invoke(mockSigner, certificate);
        } catch (UndeclaredThrowableException | NoSuchMethodException | IllegalAccessException |
