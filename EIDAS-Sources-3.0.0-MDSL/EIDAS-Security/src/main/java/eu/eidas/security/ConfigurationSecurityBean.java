...
public class ConfigurationSecurityBean {
   ...
   private String cspReportingUri;
    ...
    private Set<String> whitelistedTelemetryIps;
    ...
    public void setWhitelistedTelemetryIps(String whitelistedTelemetryIps) {
        List<String> whiteList = List.of(whitelistedTelemetryIps.split("[;,]"));

        this.whitelistedTelemetryIps = new HashSet<>(whiteList);
    }

    public Set<String> getWhitelistedIps(String type) {
        if(null != type && type.equals(WhiteListType.TELEMETRY)){
            return whitelistedTelemetryIps;
        }
        return null;
    }
}
