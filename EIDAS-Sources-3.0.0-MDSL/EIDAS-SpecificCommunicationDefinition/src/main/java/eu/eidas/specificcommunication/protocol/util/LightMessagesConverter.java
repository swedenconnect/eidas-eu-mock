...
public class LightMessagesConverter {
    ...
    public LightRequest convert(ILightRequest iLightRequest) throws SpecificCommunicationException {
        ...
        try {
            xmlLightRequest.setCitizenCountryCode(iLightRequest.getCitizenCountryCode());
            ...
            xmlLightRequest.setSpCountryCode(iLightRequest.getSpCountryCode());
    ...
    private ImmutableAttributeMap convert(LightRequest.RequestedAttributes attributes,
        ...
        if (attributes != null && !attributes.getAttribute().isEmpty()) {
            for (LightRequest.RequestedAttributes.Attribute attribute : attributes.getAttribute()) {
                ...
                } catch (URISyntaxException | AttributeValueMarshallingException e) {
                    if (e instanceof  URISyntaxException) {
                        LOG.error("\n>>>>>>>>>> Invalid attribute definition not an URI >>>>>>>>>>\n" + attribute.getDefinition());
                    } else {
    ...
    @Nonnull
    private static URI getUri(LightRequest.RequestedAttributes.Attribute attribute) throws URISyntaxException {
        String definition = attribute.getDefinition();
        URI nameUri;
        if (null != definition) {
            nameUri = new URI(definition);
        } else {
            throw new IllegalArgumentException("Attribute definition is null");
        }
    ...
    private ResponseStatus convert(LightResponse.Status responseStatus) throws SpecificCommunicationException {
        if (responseStatus == null) {
            return null;
        }
        ResponseStatus.Builder responseStatusBuilder = ResponseStatus.builder();

        if (null == responseStatus.isFailure()) {
            throw new SpecificCommunicationException("isFailure element cannot be null or blank");
        }
        ...
        try {
            status = responseStatusBuilder.build();
    ...
    private AttributeDefinition<? extends Object> getByName(URI nameUri, Collection<AttributeDefinition<?>> registry)
        ...
        LOG.error("Attribute %s not present in the registry", nameUri);
        return null;
    }
    ...
    private Iterable unmarshalValues(List<String> values, AttributeDefinition<?> definition)
        ...
        Set<AttributeValue<?>> valuesSet = new LinkedHashSet<>();
        ...
        for (Object value : values) {
            ...
            valuesSet.add(valueMarshaller.unmarshal(value.toString(), nonLatin));
        }
        return valuesSet;
    }

    private LightRequest.RequestedAttributes.Attribute createRequestAttribute(AttributeDefinition attributeDefinition,
                                                                              Set<AttributeValue> attributeValues) throws AttributeValueMarshallingException {
