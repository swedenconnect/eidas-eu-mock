...
public final class AfterCitizenConsentRequestServlet extends AbstractSpecificProxyServiceServlet {
    ...
    private void execute(@Nonnull final HttpServletRequest httpServletRequest,
        ...
        if (StringUtils.isNotEmpty(isCanceled)) {
            ...
            try {
                TokenRedirectHelper.setTokenRedirectAttributes(httpServletRequest, iLightResponse, getSpecificProxyService().getSpecificProxyserviceResponseUrl());
    ...
    private ILightRequest createConsentedIlightRequest(ILightRequest iLightRequest, CitizenConsent citizenConsent) {
        final ImmutableAttributeMap immutableAttributeMap = filterConsentedAttributes(citizenConsent, iLightRequest.getRequestedAttributes());
        LightRequest.Builder lightRequestBuilder = LightRequest.builder()
    ...
    public ImmutableAttributeMap filterConsentedAttributes(@Nonnull CitizenConsent citizenConsent,
        ...
        for (final Map.Entry<AttributeDefinition<?>, Set<? extends AttributeValue<?>>> entry : attributes.getAttributeMap()
                .entrySet()) {
            ...
            if (definition.isRequired() || citizenConsent.getOptionalList().contains(name)) {
                consentedAttributes.put(definition, (Set) values);
            } else {
