...
public class AuthnResponseGenerator {
  ...
  public ResponseData getAuthnResponse(Map<String, String[]> parameterMap, AuthenticationRequest authenticationRequest, String clientIp,
    String spCountry, boolean cancel) throws JAXBException {
    ...
    if (cancel) {
      ...
      responseData.setErrorMessage("Authentication cancelled by user");
    } else {
  ...
  private void setNameIdAndSubject(ResponseData responseData, AuthenticationRequest authenticationRequest) {
    ...
    Optional<Attribute> natId = attributeList.stream()
      .filter(attribute -> attribute.getName().equalsIgnoreCase(EidasNaturalAttributeFriendlyName.personIdentifier.getFriendlyName()))
      .findFirst();
    Optional<Attribute> legalId = attributeList.stream()
      .filter(attribute -> attribute.getName().equalsIgnoreCase(EidasLegalAttributeFriendlyName.legalPersonIdentifier.getFriendlyName()))
      .findFirst();
