...
public class ConnectorOutgoingLightResponseLoggerFilterTest {
     ...
     */
    private final Logger logger = (Logger) LoggerFactory.getLogger(ConnectorOutgoingLightResponseLoggerFilter.class.getName());

    private HttpServletRequest mockHttpServletRequest;
    private HttpServletResponse mockHttpServletResponse;
    private FilterChain mockFilterChain;
    private ConnectorOutgoingLightResponseLogger mockConnectorOutgoingLightResponseLogger;
    private ConnectorOutgoingLightResponseLoggerFilter connectorOutgoingLightResponseLoggerFilter;
    ...
    public void setup() {
        ...
        mockConnectorOutgoingLightResponseLogger = mock(ConnectorOutgoingLightResponseLogger.class);
        setUpBeanProvider(mockConnectorOutgoingLightResponseLogger);

        connectorOutgoingLightResponseLoggerFilter = new ConnectorOutgoingLightResponseLoggerFilter();
    }

    private void setUpBeanProvider(ConnectorOutgoingLightResponseLogger mockConnectorOutgoingLightResponseLogger) {
        ApplicationContext mockApplicationContext = mock(ApplicationContext.class);
        Mockito.when(mockApplicationContext.getBean(anyString())).thenReturn(mockConnectorOutgoingLightResponseLogger);
    ...
    /**
     * Test method for {@link ConnectorOutgoingLightResponseLoggerFilter#doFilter(ServletRequest, ServletResponse, FilterChain)}.
    ...
    public void doFilter() throws IOException, ServletException, SpecificCommunicationException {
        ...
        connectorOutgoingLightResponseLoggerFilter.doFilter(mockHttpServletRequest,mockHttpServletResponse, mockFilterChain);

        Logger messageLogger = getMessageLogger();
        InOrder orderVerifier = inOrder(mockFilterChain, mockConnectorOutgoingLightResponseLogger );
        orderVerifier.verify(mockFilterChain).doFilter(mockHttpServletRequest,mockHttpServletResponse);
        orderVerifier.verify(mockConnectorOutgoingLightResponseLogger).logMessage(messageLogger, mockHttpServletRequest);

        assertLogging("LightResponseOutgoingLogger FILTER for", traceAppender);
    }
    ...
    public void doFilterServletException() throws IOException, SpecificCommunicationException {
        ...
        try {
            Logger messageLogger = getMessageLogger();
            doNothing().when(mockConnectorOutgoingLightResponseLogger).logMessage(messageLogger, mockHttpServletRequest);
    ...
    public void doFilterSpecificCommunicationException() throws ServletException, IOException, SpecificCommunicationException {
        ...
        Logger messageLogger = getMessageLogger();
        doThrow(expectedException).when(mockConnectorOutgoingLightResponseLogger).logMessage(messageLogger, mockHttpServletRequest);
